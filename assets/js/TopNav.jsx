...
function TopNav({ copy, onOpenAccount, onOpenPlaylists, onOpenSearch }) {
  const f = useFithop();
  const accountLabel = f.isLoggedIn ? copy.nav_account : copy.login;
  return (
    <header className="fh-nav">
      ...
      <div className="fh-nav-side right">
        ...
        <button className="fh-nav-account" aria-label={accountLabel} title={accountLabel} onClick={onOpenAccount}><Icon.User size={19} /></button>
      </div>
    </header>);
