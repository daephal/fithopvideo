...
function FithopProvider({ children }) {
  ...
  const [selectedFitclip, setSelectedFitclipState] = React.useState(() => {
    ...
    return (n >= 1 && n <= max) ? n : Math.min(48, max);
  });
  ...
  const realIsAdmin = window.isAdminUser ? window.isAdminUser(currentUser) : (!!currentUser && currentUser.loggedIn && window.RILLIZ_DATA.auth.adminEmails.includes(currentUser.email));
  ...
  const setSelectedFitclip = (n) => { const v = Math.min(maxFitclipNumber, Math.max(1, n)); setSelectedFitclipState(v); lsSet(LS.selectedFitclip, v); };
  ...
  const value = {
    ...
    selectedFitclip, setSelectedFitclip, stepAlbum, getFitclip, currentFitclip, maxFitclipNumber, catalogVersion, refreshCatalog,
